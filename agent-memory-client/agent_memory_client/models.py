...
class SummaryViewPartitionResult(BaseModel):
    ...
    summary: str = Field(default="", description="Summarized text for this partition")
    memory_count: int = Field(
        description="Number of memories that contributed to this summary"
    )
    empty: bool = Field(
        default=False, description="Whether this partition had no matching memories"
    )
    empty_reason: str | None = Field(
        default=None, description="Machine-readable reason for an empty partition"
    )
