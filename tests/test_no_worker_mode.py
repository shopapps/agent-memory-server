...
async def test_no_worker_mode_runs_tasks_inline(
    mock_vectorstore_adapter, mock_llm_client
):
    ...
    async def mock_get_model_client(*args, **kwargs):
...
@pytest.mark.asyncio
async def test_worker_mode_queues_tasks(mock_vectorstore_adapter, mock_llm_client):
