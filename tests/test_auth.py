...
class TestJWKSCache:
    ...
    async def test_jwks_cache_http_error(self):
        ...
        with patch("httpx.Client") as mock_client:
            mock_context = Mock()
            mock_context.__enter__.return_value.get.side_effect = httpx.HTTPError("Connection failed")
            mock_client.return_value = mock_context
...
class TestJWTVerification:
    ...
    def test_verify_jwt_future_token(self, mock_settings, jwks_data):
        ...
        future_payload = {
            ...
            "iss": "https://test-issuer.com",
            "exp": int((datetime.now(timezone.utc) + timedelta(hours=1)).timestamp()),
            "iat": int((datetime.now(timezone.utc) + timedelta(hours=1)).timestamp()),  # Future
        }
...
class TestIntegrationScenarios:
    ...
    async def test_auth0_integration_scenario(self, mock_settings, jwks_data):
        ...
        with patch("agent_memory_server.auth.get_public_key") as mock_get_key:
            ...
            assert result.aud == ["https://api.myapp.com", "https://myapp.auth0.com/userinfo"]
    ...
    async def test_aws_cognito_integration_scenario(self, mock_settings, jwks_data):
        ...
        mock_settings.disable_auth = False
        mock_settings.oauth2_issuer_url = "https://cognito-idp.us-east-1.amazonaws.com/us-east-1_XXXXXXXXX"
    ...
    @pytest.mark.asyncio
    async def test_performance_token_validation(self, mock_settings, jwks_data, valid_token):
    ...
    @pytest.mark.asyncio
    async def test_concurrent_auth_requests(self, mock_settings, jwks_data, valid_token):
...
class TestErrorHandling:
    ...
    async def test_network_timeout_handling(self, mock_settings):
        ...
        with patch("httpx.Client") as mock_client:
            mock_context = Mock()
            mock_context.__enter__.return_value.get.side_effect = httpx.TimeoutException("Timeout")
            mock_client.return_value = mock_context
